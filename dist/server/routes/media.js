...
mediaRouter.post("/", requireAuth, requireRole(["ADMIN", "EDITOR"]), upload.single("asset"), validate({ body: uploadBodySchema }), (req, res, next) => getController().uploadAsset(req, res, next));
mediaRouter.delete("/:id", requireAuth, requireRole(["ADMIN", "EDITOR"]), validate({ params: deleteParamsSchema }), (req, res, next) => getController().deleteAsset(req, res, next));
