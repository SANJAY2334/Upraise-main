...
if (parsedEnv.NODE_ENV === "production") {
    ...
    if (parsedEnv.JWT_ACCESS_SECRET === "dev-access-secret-change-me") {
        console.error("❌ CRITICAL SECURITY ERROR: JWT_ACCESS_SECRET must be set to a secure custom value in production mode.");
        process.exit(1);
    }
    if (parsedEnv.JWT_REFRESH_SECRET === "dev-refresh-secret-change-me") {
        console.error("❌ CRITICAL SECURITY ERROR: JWT_REFRESH_SECRET must be set to a secure custom value in production mode.");
        process.exit(1);
    }
}
export const config = {
